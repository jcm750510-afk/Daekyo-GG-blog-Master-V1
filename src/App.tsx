...
function parseLines(raw: string, limit: number): string[] {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.map(String).map((x) => x.trim()).filter(Boolean).slice(0, limit);
    }
  } catch {
...
export default function App() {
  ...
  const openPromptInChatGPT = async (prompt: string, kind: 'topics' | 'titles') => {
  ...
  const importClipboard = async (kind: 'topics' | 'titles') => {
    try {
      ...
      if (kind === 'topics') {
        setTopicInput(raw);
        setTopics(parseLines(raw, 5));
      } else {
        setTitleInput(raw);
        setTitles(parseLines(raw, 5));
      }
    } catch {
      // Browser permission can block clipboard reading. The manual textarea remains available.
    }
  };
  ...
  const chooseTitle = (title: string) => {
    setForm((prev) => ({ ...prev, topic: prev.topic, directorRequest: prev.directorRequest }));
    setForm((prev) => ({ ...prev, topic: prev.topic }));
    setStep(4);
    void title;
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b bg-white">
        <div className="mx-auto max-w-6xl px-5 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">GPT용 눈높이 블로그 프롬프트 마스터 V3</h1>
              <p className="mt-1 text-sm text-slate-500">조건 입력 → GPT 5가지 주제 → 후킹 제목 5가지 → 1,200~1,500자 블로그</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-5 py-8">
        <div className="mb-7 grid gap-3 md:grid-cols-4">
          {[
            ['01', '콘텐츠 조건 입력', '과목·학년·요청사항'],
            ['02', 'GPT 5가지 추천주제', '버튼으로 GPT 실행'],
            ['03', '후킹 제목 5가지', '선택 주제로 제목 생성'],
            ['04', '완성 프롬프트', '1,200~1,500자 글 작성'],
        ...
        {step === 1 && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div><h2 className="text-lg font-bold">STEP 1. 블로그 기본 조건</h2><p className="text-sm text-slate-500">과목과 원장님 요청사항을 중심으로 입력하면 GPT가 주제를 기획합니다.</p></div>
            </div>
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Field label="지역 / 러닝센터명"><input value={form.region} onChange={(e) => update('region', e.target.value)} placeholder="예: 수원 매탄러닝센터" /></Field>
              ...
              <div className="md:col-span-2"><Field label="원장님 요청사항"><textarea value={form.directorRequest} onChange={(e) => update('directorRequest', e.target.value)} placeholder="예: 초3 학부모가 바로 공감할 수 있게, 연산이 부족한 아이의 지도 방법을 구체적으로" rows={4} /></Field></div>
            </div>
            <div className="mt-7 flex justify-end"><button onClick={() => setStep(2)} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-700">GPT 5가지 추천주제로 이동 <Wand2 size={17} /></button></div>
          </section>
        )}
          ...
          <section className="space-y-5">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-start justify-between gap-4">
                <div className="flex gap-3"><Lightbulb className="mt-1" size={22} /><div><h2 className="text-lg font-bold">STEP 2. GPT 5가지 추천주제</h2><p className="mt-1 text-sm text-slate-500">기존 10개 복사·붙여넣기 방식을 없애고, 지금 입력한 조건으로 GPT가 5개만 기획합니다.</p></div></div>
              ...
              {openGuide && <div className="mb-5 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">① <b>GPT 5가지 추천주제</b> 버튼을 누릅니다. → ② GPT가 5개를 제안합니다. → ③ GPT 결과를 복사하면 아래 <b>결과 불러오기</b> 버튼으로 한 번에 표시할 수 있습니다. → ④ 주제를 클릭하면 후킹 제목 단계로 이동합니다.</div>}
              <div className="flex flex-wrap gap-2">
                <button onClick={() => void openPromptInChatGPT(topicPrompt, 'topics')} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">{busy === 'topics' ? <Check size={16} /> : <Sparkles size={16} />} GPT 5가지 추천주제</button>
                <button onClick={() => void handleCopy(topicPrompt, 'topicPrompt')} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold">{copied === 'topicPrompt' ? <Check size={16} /> : <Copy size={16} />} 프롬프트 복사</button>
                <button onClick={() => void importClipboard('topics')} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold"><ClipboardPaste size={16} /> GPT 결과 불러오기</button>
              </div>
            </div>

            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between gap-3"><div><h3 className="font-bold">추천 주제 5개</h3><p className="mt-1 text-sm text-slate-500">GPT 결과가 자동 인식됩니다. 필요하면 아래 입력창에 붙여넣어도 됩니다.</p></div><span className="text-xs text-slate-400">현재 {topics.length}개</span></div>
              <textarea value={topicInput} onChange={(e) => { setTopicInput(e.target.value); setTopics(parseLines(e.target.value, 5)); }} rows={5} placeholder={'GPT 결과를 붙여넣으면 5개 주제로 자동 정리됩니다.'} />
              {topics.length > 0 && <div className="mt-4 grid gap-3">{topics.map((topic, index) => <button key={`${topic}-${index}`} onClick={() => chooseTopic(topic)} className="flex gap-4 rounded-2xl border border-slate-200 p-4 text-left transition hover:border-slate-900 hover:bg-slate-50"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-bold">{index + 1}</span><span className="text-sm font-medium leading-6">{topic}</span></button>)}</div>}
            </div>
            <div className="flex justify-between"><button onClick={() => setStep(1)} className="rounded-xl border px-4 py-2.5 text-sm font-semibold">이전</button><button disabled={!form.topic} onClick={() => setStep(3)} className="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">선택 주제로 후킹 제목 만들기</button></div>
          </section>
        )}
        ...
        {step === 3 && (
          <section className="space-y-5">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-start justify-between gap-4">
                <div className="flex gap-3"><Lightbulb className="mt-1" size={22} /><div><h2 className="text-lg font-bold">STEP 3. 후킹 제목 5가지</h2><p className="mt-1 text-sm text-slate-500">선택한 주제 하나를 바탕으로 서로 다른 각도의 제목 5개를 생성합니다.</p></div></div>
                <div className="rounded-xl bg-slate-50 px-4 py-3 text-sm font-semibold">{form.topic || '주제를 먼저 선택하세요'}</div>
              </div>
              <div className="flex flex-wrap gap-2">
                <button disabled={!form.topic} onClick={() => void openPromptInChatGPT(titlePrompt, 'titles')} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">{busy === 'titles' ? <Check size={16} /> : <Sparkles size={16} />} 후킹 제목 5가지 생성</button>
                <button onClick={() => void handleCopy(titlePrompt, 'titlePrompt')} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold">{copied === 'titlePrompt' ? <Check size={16} /> : <Copy size={16} />} 프롬프트 복사</button>
                <button onClick={() => void importClipboard('titles')} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold"><ClipboardPaste size={16} /> GPT 결과 불러오기</button>
              </div>
            </div>
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between"><div><h3 className="font-bold">후킹 제목 5개</h3><p className="mt-1 text-sm text-slate-500">제목을 클릭하면 최종 블로그 프롬프트에 반영됩니다.</p></div><span className="text-xs text-slate-400">현재 {titles.length}개</span></div>
              <textarea value={titleInput} onChange={(e) => { setTitleInput(e.target.value); setTitles(parseLines(e.target.value, 5)); }} rows={5} placeholder={'GPT가 만든 제목 5개를 붙여넣으면 자동 정리됩니다.'} />
              {titles.length > 0 && <div className="mt-4 grid gap-3">{titles.map((title, index) => <button key={`${title}-${index}`} onClick={() => { update('topic', form.topic); setForm((prev) => ({ ...prev, directorRequest: `${prev.directorRequest}${prev.directorRequest ? '\n' : ''}확정 제목: ${title}` })); setStep(4); }} className="flex gap-4 rounded-2xl border border-slate-200 p-4 text-left transition hover:border-slate-900 hover:bg-slate-50"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-bold">{index + 1}</span><span className="text-sm font-medium leading-6">{title}</span></button>)}</div>}
            </div>
            <div className="flex justify-between"><button onClick={() => setStep(2)} className="rounded-xl border px-4 py-2.5 text-sm font-semibold">주제 다시 선택</button><button disabled={titles.length === 0} onClick={() => setStep(4)} className="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">최종 블로그 프롬프트 만들기</button></div>
          </section>
        )}

        {step === 4 && (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-5 flex items-center justify-between gap-4"><div className="flex gap-3"><FileText className="mt-1" size={22} /><div><h2 className="text-lg font-bold">STEP 4. 1,200~1,500자 블로그 작성 프롬프트</h2><p className="mt-1 text-sm text-slate-500">AI 느낌을 줄이고, 문단과 연결된 이미지 프롬프트를 본문 사이에 배치하도록 설계했습니다.</p></div></div><button onClick={() => void handleCopy(finalPrompt, 'final')} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white">{copied === 'final' ? <Check size={16} /> : <Copy size={16} />} {copied === 'final' ? '복사 완료' : '최종 프롬프트 복사'}</button></div>
            <div className="mb-5 grid gap-3 md:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs text-slate-500">선택한 주제</div><div className="mt-1 font-semibold">{form.topic || '아직 주제를 선택하지 않았습니다.'}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs text-slate-500">본문 기준</div><div className="mt-1 font-semibold">공백 포함 1,200~1,500자 · 이미지 2~3개 · 교육 90 : 홍보 10</div></div></div>
            <pre className="max-h-[680px] overflow-auto whitespace-pre-wrap rounded-2xl border border-slate-200 bg-white p-5 text-sm leading-7">{finalPrompt}</pre>
            <div className="mt-5 flex flex-wrap justify-between gap-3"><button onClick={() => setStep(3)} className="rounded-xl border px-4 py-2.5 text-sm font-semibold">제목 다시 선택</button><button onClick={() => void handleCopy(finalPrompt, 'final2')} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white">{copied === 'final2' ? <Check size={16} /> : <Copy size={16} />} GPT에 붙여넣기</button></div>
          </section>
        )}
      </main>
